class EventDetails {
  constructor() {
    ...
    this.type = JSON.parse(sessionStorage.getItem("_selectedCategories"));
...
if (filename === "add_event_1.html") {
  ...
  document.getElementById("onlineCheckbox").addEventListener("change", function () {
    ...
    if (!_linkInput.disabled) {
      ...
      _linkInput.value = ""; // Clear the input value when turning online mode off
      // Store the cleared value in session storage
      sessionStorage.setItem("_linkInput", _linkInput.value);
    }
  });
...
function loadSavedData() {
  if (filename === "add_event_1.html") {
      ...
      let categoryCount = sessionStorage.getItem("categoryCount")
...
function validateAndNavigate() {
  if (filename === "add_event_1.html") {
      ...
      sessionStorage.setItem("_selectedCategories", JSON.stringify(_selectedCategories));
